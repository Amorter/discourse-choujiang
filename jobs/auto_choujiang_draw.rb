  ...
  class AutoChoujiangDraw < ::Jobs::Scheduled
    ...
    def execute(args)
      ...
      topics.each do |topic|
        ...
        Rails.logger.warn("choujiang: topic #{topic.id} parsed info: #{info.inspect}")
        unless info
        ...
        unless info[:draw_time] && Time.now >= info[:draw_time]
          Rails.logger.warn("choujiang: topic #{topic.id} not time to draw or missing draw_time (draw_time=#{info[:draw_time]}, now=#{Time.now}), skip")
          next
        end
